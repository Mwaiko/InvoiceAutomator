...
def extract_grn_from_image(image_path: str) -> dict:
    """Run OCR on *image_path* and return a GRN dict identical in structure
    to the one returned by read_pdf.extract_grn()."""

    ocr    = _get_ocr()
    result = ocr.predict(input=image_path)

    # Collect all recognised text tokens in reading order
    tokens: list[str] = []
    for res in result:
        texts = res.get("rec_texts", [])
        tokens.extend(t.strip() for t in texts if t.strip())
